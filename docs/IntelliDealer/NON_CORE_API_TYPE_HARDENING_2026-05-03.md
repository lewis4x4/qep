...
| Executive shared data hook | Metric definitions, latest KPI snapshots, and analytics alerts were returned directly from unknown Supabase/RPC payloads. | Added exported exec normalizers with required field checks, role/status/severity/refresh-state validation, numeric-string coercion, string-array cleanup, and metadata/config object guards. |
...
- Exec shared data normalizer tests: `4 pass`, `0 fail`.
- Combined targeted test run: `30 pass`, `0 fail`.
- Web typecheck: PASS.
...
- Prioritize pages where malformed rows can break operator flows: executive drill drawers/components and high-traffic floor widgets.
