...
8. **Fulfillment run audit (`/service/fulfillment/:runId`):** from **Portal → Parts orders** expanded row, **Service job drawer → Find portal order** search hit, or **Parts work queue** row (when the job is linked to a run) — **`fulfillment_run_id`** opens the same audit page; vendor dedupe shows **`idempotency_key`** when set.
