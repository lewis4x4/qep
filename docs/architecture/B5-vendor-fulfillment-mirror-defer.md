...
- **UI (staff):** **`/service/fulfillment/:runId`** — channel badges, short event titles, **`idempotency_key`** when set (vendor retries), raw JSON. Entry points when `fulfillment_run_id` exists: **Portal → Parts orders** (expanded row), **Service job drawer** (linked run + portal order search hits), **Parts work queue** row link, **Service command center** Parts hub strip (job context).
