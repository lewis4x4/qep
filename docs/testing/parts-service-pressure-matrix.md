...
- Always (no secrets): `bun run pressure:parts` — filesystem, `migrations:check`, grep guards (planner/mirror/vendor paths, **`parseVendorInboundContract`**, **`idempotencyKey`** on mirror, **`logServiceCronRun`** on vendor-escalator, **`RESEND_API_KEY`** / **`api.resend.com`** on **`_shared/vendor-escalation-resend.ts`**, **`usePartsQueue`** + **`PartsQueueBucket`** fulfillment run wiring, standalone **`PartsSubNav`** + **`parts-order-manager`** `create_internal_order` / `advance_status` / `pick_order_line`, **`CatalogSearchBar`**, **`CounterSaleForm`**, **`VendorMetricsCard`**, **`ServicePartsHubStrip`** `/parts` cross-link). **GitHub Actions** `CI` workflow runs this before **`bun run build`**. Root **`bun run build`** also runs **`verify:parts-contracts`**, **`verify:service-cron-logging`**, and **`test:vendor-inbound-contract`**.
