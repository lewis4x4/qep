...
const sql = readdirSync("supabase/migrations").filter((f) => /^84[0-5]_.*\.behavior\.test\.ts$|^839_.*\.behavior\.test\.ts$/.test(f)).sort().map((f) => `supabase/migrations/${f}`);
if (sql.length !== 7) throw new Error("Expected all seven remediation database behavior suites.");
