...
async function verifyProductionBrowser() {
  ...
  try {
    ...
    await smokeRoute(page, "/floor", "floor-production-desktop.png", ["THE FLOOR"], ["Sales Quote Flow Redesign", "Trade-In", "Financing"]);
    await desktop.close();
    ...
    await smokeRoute(mobilePage, "/floor", "floor-production-mobile.png", ["THE FLOOR"], ["Sales Quote Flow Redesign", "Trade-In", "Financing"]);
    await mobile.close();
  } finally {
