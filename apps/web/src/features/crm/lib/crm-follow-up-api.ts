...
import type {
  CrmEnrollmentStatus,
  CrmFollowUpSequence,
  CrmFollowUpSequenceEditorInput,
  CrmFollowUpStep,
  CrmSequenceEnrollment,
} from "./types";

type SequenceRow = CrmDatabase["public"]["Tables"]["follow_up_sequences"]["Row"];
type StepRow = CrmDatabase["public"]["Tables"]["follow_up_steps"]["Row"];
type EnrollmentRow = CrmDatabase["public"]["Tables"]["sequence_enrollments"]["Row"];
...
function toStep(row: StepRow): CrmFollowUpStep {
  return {
    id: row.id,
...
async function loadSequenceSteps(sequenceIds: string[]): Promise<Map<string, CrmFollowUpStep[]>> {
  ...
  const stepMap = new Map<string, CrmFollowUpStep[]>();
...
async function getCrmFollowUpSequence(sequenceId: string): Promise<CrmFollowUpSequence> {
...
function toSequenceFromRpc(payload: unknown): CrmFollowUpSequence {
  ...
  const steps = Array.isArray(record.steps)
    ? record.steps.map((step) => {
        ...
        return {
          ...
          stepType: stepRecord.stepType as CrmFollowUpStep["stepType"],
...
export async function listCrmFollowUpSequences(): Promise<CrmFollowUpSequence[]> {
...
export async function saveCrmFollowUpSequence(
  input: CrmFollowUpSequenceEditorInput,
  userId: string,
): Promise<CrmFollowUpSequence> {
    ...
    rpc: (
      ...
      args: {
        ...
        p_steps: Array<{
          stepNumber: number;
          dayOffset: number;
          stepType: CrmFollowUpStep["stepType"];
          subject: string | null;
...
export async function listCrmSequenceEnrollments(): Promise<CrmSequenceEnrollment[]> {
...
export async function updateCrmSequenceEnrollmentStatus(
  ...
  const updates: CrmDatabase["public"]["Tables"]["sequence_enrollments"]["Update"] = {
    status,
  };
