import { crmSupabase } from "./crm-supabase";
import { dealCompositeQueryKey } from "./deal-composite-keys";
import type { Cadence, CadenceTouchpoint, CrmDealDemoSummary, NeedsAssessment } from "./deal-composite-types";
import type { CrmActivityItem, CrmCompanySummary, CrmContactSummary, CrmDealLossFields, CrmRepSafeDeal } from "./types";
...
export interface DealCompositeBundle {
  ...
  cadences: Cadence[];
  demos: CrmDealDemoSummary[];
  activities: CrmActivityItem[];
  lossFields: CrmDealLossFields;
}
...
function mapDealFromCompositeJson(d: Record<string, unknown>): CrmRepSafeDeal {
  return {
    id: d.id as string,
...
function mapContactFromJson(j: Record<string, unknown>): CrmContactSummary {
  return {
    id: j.id as string,
...
function mapCompanyFromJson(j: Record<string, unknown>): CrmCompanySummary {
  return {
    id: j.id as string,
    workspaceId: j.workspace_id as string,
...
function mapActivityFromJson(row: Record<string, unknown>): CrmActivityItem {
  const meta = row.metadata;
  return {
    id: row.id as string,
    workspaceId: row.workspace_id as string,
    activityType: row.activity_type as CrmActivityItem["activityType"],
...
function normalizeDemo(raw: Record<string, unknown>): CrmDealDemoSummary {
  const { inspections: _i, ...rest } = raw;
  return rest as unknown as CrmDealDemoSummary;
}
...
export async function fetchDealComposite(dealId: string): Promise<DealCompositeBundle | null> {
  ...
  const lossFields: CrmDealLossFields = isRecord(lossJson)
    ? {
  ...
  const demos: CrmDealDemoSummary[] = Array.isArray(demosJson)
    ? demosJson.filter(isRecord).map((d) => normalizeDemo(d))
    : [];

  const activities: CrmActivityItem[] = Array.isArray(activitiesJson)
