import type { CrmActivityItem } from "./types";

/** Mirrors `buildCrmSummary` in `supabase/functions/_shared/voice-capture-crm.ts`. */
export type CrmVoiceCaptureExtractedSummary = {
...
export type CrmVoiceCaptureTimelineSignals = {
  summary: CrmVoiceCaptureExtractedSummary;
  actionItems: string[];
};
...
function parseExtractedSummary(raw: unknown): CrmVoiceCaptureExtractedSummary | null {
...
export function isVoiceCaptureActivity(activity: CrmActivityItem): boolean {
  const m = activity.metadata;
  if (!m || typeof m !== "object") return false;
  return (m as Record<string, unknown>).source === "voice_capture";
}

export function readVoiceCaptureTimelineSignals(activity: CrmActivityItem): CrmVoiceCaptureTimelineSignals | null {
...
export function voiceCaptureSignalsHaveContent(signals: CrmVoiceCaptureTimelineSignals): boolean {
