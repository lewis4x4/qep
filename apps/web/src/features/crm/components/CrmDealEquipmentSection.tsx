...
import type { CrmDealEquipmentRole, CrmEquipment } from "../lib/types";

interface CrmDealEquipmentSectionProps {
  dealId: string;
  companyId: string | null;
}

const ROLE_OPTIONS: { value: CrmDealEquipmentRole; label: string }[] = [
...
export function CrmDealEquipmentSection({ dealId, companyId }: CrmDealEquipmentSectionProps) {
  const queryClient = useQueryClient();
  const { toast } = useToast();
  const [sheetOpen, setSheetOpen] = useState(false);
  const [selectedEquipmentId, setSelectedEquipmentId] = useState("");
  const [selectedRole, setSelectedRole] = useState<CrmDealEquipmentRole>("subject");
  ...
  const companyEquipmentQuery = useQuery({
    ...
    queryFn: () => (companyId ? fetchCompanyEquipment(companyId) : Promise.resolve([] as CrmEquipment[])),
    staleTime: 30_000,
            ...
            <div className="mt-4 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className={selectClass}
                  value={selectedRole}
                  onChange={(e) => setSelectedRole(e.target.value as CrmDealEquipmentRole)}
                >
                  {ROLE_OPTIONS.map((o) => (
