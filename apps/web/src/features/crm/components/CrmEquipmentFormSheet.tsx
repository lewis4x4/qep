...
import type {
  CrmEquipment,
  CrmEquipmentCategory,
  CrmEquipmentCondition,
  CrmEquipmentAvailability,
  CrmEquipmentOwnership,
} from "../lib/types";

const CATEGORY_OPTIONS: { value: CrmEquipmentCategory; label: string }[] = [
...
const CONDITION_OPTIONS: { value: CrmEquipmentCondition; label: string }[] = [
  { value: "new", label: "New" },
...
const AVAILABILITY_OPTIONS: { value: CrmEquipmentAvailability; label: string }[] = [
...
const OWNERSHIP_OPTIONS: { value: CrmEquipmentOwnership; label: string }[] = [
...
function draftFromEquipment(eq: CrmEquipment): EquipmentDraft {
  return {
    name: eq.name,
    make: eq.make ?? "",
    model: eq.model ?? "",
...
interface CrmEquipmentFormSheetProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  existing?: CrmEquipment | null;
  isPending: boolean;
  onSubmit: (payload: ReturnType<typeof draftToPayload>) => void;
}

export function CrmEquipmentFormSheet({
  open,
  onOpenChange,
  existing,
  isPending,
  onSubmit,
}: CrmEquipmentFormSheetProps) {
        ...
        <div className="mt-4 space-y-3">
          ...
          <FieldRow>
            ...
            <div>
              ...
              <select id="eq-category" className={selectClass} value={draft.category} onChange={(e) => update("category", e.target.value as CrmEquipmentCategory)}>
          ...
          <FieldRow>
            <div>
              ...
              <select id="eq-condition" className={selectClass} value={draft.condition} onChange={(e) => update("condition", e.target.value as CrmEquipmentCondition)}>
                <option value="">Select…</option>
            ...
            <div>
              ...
              <select id="eq-availability" className={selectClass} value={draft.availability} onChange={(e) => update("availability", e.target.value as CrmEquipmentAvailability)}>
          ...
          <div>
            ...
            <select id="eq-ownership" className={selectClass} value={draft.ownership} onChange={(e) => update("ownership", e.target.value as CrmEquipmentOwnership)}>
