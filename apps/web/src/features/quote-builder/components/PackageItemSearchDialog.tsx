...
// WAVE quote-builder deep reflow (B1): swap to MobileBottomSheet on phones.
    ...
    <div className="space-y-4">
      <div className="grid gap-2 md:grid-cols-[1fr_auto]">
        <input
          ...
          autoFocus
        />
        <Button onClick={() => searchMutation.mutate(query)} disabled={searchMutation.isPending} className="min-h-[44px]">
  ...
  if (isMobile) {
    return (
      <MobileBottomSheet
        ...
        title={`Add ${kindLabel.toLowerCase()}`}
        description="Search catalog-backed package items so the quote gets the right name, price, and source trail."
        size="tall"
      >
        {searchBody}
      </MobileBottomSheet>
