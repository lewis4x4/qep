...
import { listCrmActivityTemplates } from "../lib/qrm-api";
import type { QrmActivityType, QrmTaskMetadata, QrmTaskStatus } from "../lib/types";

interface QrmActivityComposerProps {
  open: boolean;
...
const ACTIVITY_OPTIONS: Array<{ value: QrmActivityType; label: string }> = [
...
export function QrmActivityComposer({
  open,
  onOpenChange,
  onSubmit,
  isPending,
  ...
  const [taskStatus, setTaskStatus] = useState<QrmTaskStatus>("open");
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    let normalizedTask: QrmTaskMetadata | undefined;
    if (isTaskType) {
      if (taskDueAt) {
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div>
            ...
            <select
              ...
              value={activityType}
              onChange={(event) => {
                setActivityType(event.target.value as QrmActivityType);
                setSelectedTemplateId(null);
              }}
            ...
            <div className="grid gap-4 rounded-md border border-border bg-muted/30 p-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <select
                  ...
                  value={taskStatus}
                  onChange={(event) => {
                    setTaskStatus(event.target.value as QrmTaskStatus);
                    setSelectedTemplateId(null);
                  }}
                  className="h-11 w-full rounded-md border border-input bg-card px-3 text-sm text-foreground shadow-sm focus:border-primary focus:outline-none"
                >
