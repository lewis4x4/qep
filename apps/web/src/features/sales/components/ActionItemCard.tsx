  ...
  async function handleQuickAction() {
    ...
    try {
      await logSalesActivity({
        activityType: cta.icon === "phone" ? "call" : "email",
        dealId: deal.deal_id,
        companyId: deal.company_id,
      });
    } catch {
      toast({
        title: `${cta.icon === "phone" ? "Call" : "Email"} log not saved`,
        description: "The action can continue, but activity logging failed.",
        variant: "destructive",
      });
    }
  }

  function openActionContext() {
    if (deal?.company_id) navigate(`/sales/customers/${deal.company_id}`);
    else if (action.deal_id) navigate("/sales/pipeline");
  }

  return (
    <div className="w-full text-left relative bg-[hsl(var(--card))] rounded-xl border border-white/[0.06] overflow-hidden transition-all duration-200 hover:border-white/20 hover:shadow-lg hover:shadow-black/20">
      ...
      <button
        ...
        aria-label={`Open action context for ${action.customer_name ?? "priority item"}`}
      >
      ...
      {/* Action buttons */}
      <div className="flex items-center gap-2 mt-3 pl-5 pr-4 pb-4">
          <button
            type="button"
          ...
          <button
            type="button"
            onClick={() => undefined}
            aria-label="Snooze action (not yet available)"
            className="px-4 py-2.5 bg-foreground/[0.06] text-muted-foreground text-sm font-medium rounded-lg active:scale-[0.98] transition-transform"
          >
            Snooze
          </button>
        </div>
