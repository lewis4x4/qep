...
export async function resetPasswordForEmailWithRetry(params: {
  email: string;
  redirectTo: string;
}): Promise<{ error: AuthError | null }> {
  let lastError: AuthError | null = null;
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
    ...
    try {
      const { error } = await withTimeout(
        supabase.auth.resetPasswordForEmail(params.email, {
...
export async function updatePasswordWithRetry(params: {
  password: string;
}): Promise<{ error: AuthError | null }> {
  let lastError: AuthError | null = null;
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
    try {
      const { error } = await withTimeout(
        supabase.auth.updateUser({ password: params.password }),
        AUTH_ATTEMPT_TIMEOUT_MS,
      );
      if (!error) {
        return { error: null };
