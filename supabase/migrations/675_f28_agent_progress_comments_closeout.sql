...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%supabase/migrations/675_f28_agent_progress_comments_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'QEP (1)/QEP_COMMENT_DRIVEN_AGENTS.md') ||
      ...
      ' | supabase/migrations/675_f28_agent_progress_comments_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '675_f28_agent_progress_comments_closeout.sql',
