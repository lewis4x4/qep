-- ============================================================================
-- Migration 231: Time Bank customer-delivery additive contract
-- ============================================================================

drop function if exists public.qrm_time_bank(text, integer);
...
begin
  ...
  with base as (
    select
      ...
      coalesce(nullif(ceil(coalesce(s.sla_minutes, 0)::numeric / 1440.0), 0)::integer, greatest(coalesce(p_default_budget_days, 14), 1)) as budget_days,
  ...
  select
    ...
    b.stage_id,
    b.stage_name,
    b.stage_age_days as days_in_stage,
    b.stage_age_days,
    b.budget_days,
    b.has_explicit_budget,
    greatest(b.budget_days - b.stage_age_days, 0) as remaining_days,
    round(
      case
        when b.budget_days > 0 then (b.stage_age_days::numeric / b.budget_days::numeric)
        else 1
      end,
      2
    ) as pct_used,
    (b.stage_age_days > b.budget_days) as is_over,
    greatest(b.stage_age_days - b.budget_days, 0) as overrun_days,
    case when b.has_explicit_budget then 'stage_sla' else 'fallback' end as budget_source,
    case
      when b.stage_age_days > b.budget_days then 'over'
      when b.budget_days > 0 and (b.stage_age_days::numeric / b.budget_days::numeric) >= 0.85 then 'critical'
      when b.budget_days > 0 and (b.stage_age_days::numeric / b.budget_days::numeric) >= 0.65 then 'watch'
      else 'healthy'
    end as pressure_tier
  from base b
  order by is_over desc, pct_used desc, b.stage_age_days desc, b.deal_name asc;
end;
$$;
