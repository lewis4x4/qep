-- ============================================================================
-- Migration 092: Security Lockdown
--
-- CRITICAL: Fixes cross-workspace data leak in get_deal_composite().
-- Also adds workspace isolation improvements and schema hardening.
--
-- Audit ref: QEP-OS-Code-Audit-Report.md, items P0-1 through P0-3
-- ============================================================================

-- ── Fix 1: deal_composite workspace check (CRITICAL) ───────────────────────
-- The existing function retrieves deals by ID only — no workspace filter.
-- Any authenticated user could read any deal by guessing UUIDs.
...
declare
  ...
  v_loss_fields jsonb;
begin
  -- CRITICAL: Get caller's workspace and verify deal belongs to it
  ...
  -- Needs Assessment (latest, workspace-scoped via deal)
