...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%752_f41_precedent_similarity_matching_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'supabase/migrations/597_qep_stream_f_decision_velocity.sql F4.1') ||
      ...
      ' | supabase/migrations/752_f41_precedent_similarity_matching_closeout.test.ts' ||
      ' | supabase/migrations/752_f41_precedent_similarity_matching_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '752_f41_precedent_similarity_matching_closeout.sql',
