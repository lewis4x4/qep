...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%supabase/migrations/671_g121_cycle_counts_dead_stock.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'QEP_PHASE3_PARTS_BLUEPRINT_2026-05-26.md §1.12, §4 ADR-019') ||
      ' | supabase/migrations/671_g121_cycle_counts_dead_stock.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '671_g121_cycle_counts_dead_stock.sql',
