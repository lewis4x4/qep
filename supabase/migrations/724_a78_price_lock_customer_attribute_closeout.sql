...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%724_a78_price_lock_customer_attribute_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'QEP-OEM-Price-Feeds-Discovery-FILLED.docx') ||
      ...
      ' | supabase/migrations/724_a78_price_lock_customer_attribute_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '724_a78_price_lock_customer_attribute_closeout.sql',
