...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%supabase/migrations/682_h71_hauling_transport_dispatch.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'QEP_SERVICE_ROADMAP_ALIGNMENT_2026-05-29.md Appendix A H7') ||
      ' | supabase/migrations/682_h71_hauling_transport_dispatch.sql' ||
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '682_h71_hauling_transport_dispatch.sql',
