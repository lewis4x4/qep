...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%763_f51_tiered_audit_trail_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'supabase/migrations/597_qep_stream_f_decision_velocity.sql F5.1') ||
      ...
      ' | supabase/migrations/763_f51_tiered_audit_trail_closeout.test.ts' ||
      ' | supabase/migrations/763_f51_tiered_audit_trail_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '763_f51_tiered_audit_trail_closeout.sql',
