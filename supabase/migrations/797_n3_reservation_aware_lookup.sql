-- ============================================================================
-- Migration 797: N3.1 — reservation-aware counter availability
--
--   parts_lookup_engine (m673) computed stock_on_hand as the raw sum of
--   parts_stock.qty_on_hand, double-counting units a service job reserved.
--   The counter's availability now excludes reservations
--   (on_hand − reserved) so "a service requirement reserves the last unit
--   and the counter sees it unavailable" holds. Function body is otherwise
--   verbatim m673 (per-location stock_locations JSON still exposes the raw
--   qty_on_hand and qty_reserved for detail views).
-- ============================================================================
...
AS $$
  ...
  stock AS (
    SELECT
      s.part_id,
      coalesce(sum(s.qty_on_hand), 0) - coalesce(sum(s.qty_reserved), 0) AS stock_on_hand,
      coalesce(
        jsonb_agg(
          jsonb_build_object(
