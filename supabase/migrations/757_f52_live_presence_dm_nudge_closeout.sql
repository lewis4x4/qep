...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%757_f52_live_presence_dm_nudge_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'supabase/migrations/597_qep_stream_f_decision_velocity.sql F5.2') ||
      ...
      ' | supabase/migrations/744_f15_brian_triage_queue_closeout.sql' ||
      ' | supabase/migrations/748_f25_decisions_web_fallback_closeout.sql' ||
      ' | supabase/migrations/757_f52_live_presence_dm_nudge_closeout.test.ts' ||
      ' | supabase/migrations/757_f52_live_presence_dm_nudge_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '757_f52_live_presence_dm_nudge_closeout.sql',
    ...
    'implementation_evidence', jsonb_build_array(
      ...
      'supabase/migrations/744_f15_brian_triage_queue_closeout.sql and supabase/migrations/748_f25_decisions_web_fallback_closeout.sql already pin the command-surface and owner-open source evidence'
    ),
