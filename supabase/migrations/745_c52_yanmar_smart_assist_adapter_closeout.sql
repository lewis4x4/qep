...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%745_c52_yanmar_smart_assist_adapter_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'JAR-107 packet') ||
      ...
      ' | supabase/migrations/745_c52_yanmar_smart_assist_adapter_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '745_c52_yanmar_smart_assist_adapter_closeout.sql',
    ...
    'dependency_evidence', jsonb_build_array(
      'C5.1 shipped in supabase/migrations/744_c51_telematics_adapter_contract_closeout.sql',
