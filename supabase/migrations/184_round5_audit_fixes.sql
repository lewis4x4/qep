-- ============================================================================
-- Migration 184: Round 5 audit fixes
--
-- Defense-in-depth on the documents_unclassified audit class:
-- equipment_documents already has workspace-scoped RLS via the
-- equipment_docs_internal policy, but run_data_quality_audit is
-- SECURITY INVOKER so the SELECT relies on the policy alone. Adding an
-- explicit workspace_id filter to the query makes the intent legible
-- and survives any future RLS policy change.
--
-- Other audit classes already filter implicitly (qrm_companies and
-- qrm_equipment carry workspace_id and have workspace-scoped RLS).
-- Only the equipment_documents path needed the explicit predicate.
-- ============================================================================
...
begin
  ...
  -- 10. Equipment with stale telematics
  insert into public.admin_data_issues (issue_class, severity, entity_table, entity_id, detail, last_checked)
  ...
  -- Round-5 audit fix: explicit workspace_id filter for defense-in-depth.
  -- equipment_docs_internal RLS already scopes by workspace, but the
  -- explicit predicate makes intent legible and survives policy changes.
  insert into public.admin_data_issues (issue_class, severity, entity_table, entity_id, detail, last_checked)
  ...
  -- 12. Open quotes with no tax jurisdiction
  insert into public.admin_data_issues (issue_class, severity, entity_table, entity_id, detail, last_checked)
...
comment on function public.run_data_quality_audit() is
  'Full 12-class data-quality scan. Round-5 fix: documents_unclassified now has an explicit workspace_id filter for defense-in-depth.';
