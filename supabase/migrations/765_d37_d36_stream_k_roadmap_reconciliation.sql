...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%765_d37_d36_stream_k_roadmap_reconciliation.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'docs/architecture/parts-pricing-ruleset.md') ||
      ' | supabase/migrations/676_g81_parts_pricing_engine_counter_discount_cap.sql' ||
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '765_d37_d36_stream_k_roadmap_reconciliation.sql',
    ...
    'implementation_evidence', jsonb_build_array(
      ...
      'supabase/migrations/676_g81_parts_pricing_engine_counter_discount_cap.sql implements list/customer/volume precedence, target/floor metadata, 5 percent counter cap, approval blocking, and pricing snapshots'
    ),
...
SET
  ...
  notes = CASE
    WHEN COALESCE(notes, '') LIKE '%[2026-07-03] K1.1 unblocked%'
      THEN notes
    ELSE COALESCE(notes, '') ||
      E'\n[2026-07-03] K1.1 unblocked: Ryan + Tina finance evidence confirms QEP OS as the forward accounting SoR, IntelliDealer as transition SoR, and QuickBooks Desktop as downstream check-register/CPA-reporting output. Migrations 662-670 provide build-now foundation. Remaining open business values are config/working-session items, not blockers to the SoR decision; do not hard-code them.'
  END,
  updated_at = now()
WHERE task_id = 'K1.1';
