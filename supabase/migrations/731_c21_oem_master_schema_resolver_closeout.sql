...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%731_c21_oem_master_schema_resolver_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'JAR-105 packet') ||
      ...
      ' | supabase/migrations/731_c21_oem_master_schema_resolver_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '731_c21_oem_master_schema_resolver_closeout.sql',
    ...
    'implementation_evidence', jsonb_build_array(
      ...
      'supabase/migrations/716_a71_oem_price_sheet_schema_closeout.sql already verifies this schema substrate for the A7 OEM price-sheet schema row'
    ),
