...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%758_f33_authorize_two_party_signing_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'supabase/migrations/597_qep_stream_f_decision_velocity.sql F3.3') ||
      ...
      ' | supabase/migrations/758_f33_authorize_two_party_signing_closeout.test.ts' ||
      ' | supabase/migrations/758_f33_authorize_two_party_signing_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '758_f33_authorize_two_party_signing_closeout.sql',
    ...
    'implementation_evidence', jsonb_build_array(
      ...
      'supabase/migrations/758_f33_authorize_two_party_signing_closeout.test.ts pins the ledger, RPC guards, completion behavior, status view, grants, and live/manual boundaries'
    ),
