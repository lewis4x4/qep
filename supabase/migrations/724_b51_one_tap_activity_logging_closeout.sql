...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%724_b51_one_tap_activity_logging_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'QEP-OMI-CONSOLIDATED-BUILD-PLAN.md DH-3') ||
      ...
      ' | supabase/migrations/724_b51_one_tap_activity_logging_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '724_b51_one_tap_activity_logging_closeout.sql',
