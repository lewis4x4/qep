...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%735_a51_fuzzy_phone_search_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'QEP-OMI-CONSOLIDATED-BUILD-PLAN.md QB-3') ||
      ...
      ' | supabase/migrations/725_b52_customer_attach_search_closeout.sql' ||
      ' | test-results/agent-gates/20260520T230316Z-A5.1-qb-3-fuzzy-phone-customer-search.json' ||
      ' | supabase/migrations/735_a51_fuzzy_phone_search_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '735_a51_fuzzy_phone_search_closeout.sql',
    ...
    'dependency_evidence', jsonb_build_object(
      'B5.2', 'shipped by supabase/migrations/725_b52_customer_attach_search_closeout.sql'
    ),
