...
SELECT DISTINCT ON (pc.workspace_id, lower(pc.part_number))
  ...
  'parts_catalog',
  jsonb_build_object(
    'migration', '671_phase3_parts_schema_foundation',
...
SELECT
  ...
  b.id,
  b.slug,
  jsonb_build_object('migration', '671_phase3_parts_schema_foundation', 'source', 'branches')
...
SELECT DISTINCT
  ...
  'branch',
  pi.branch_id,
  jsonb_build_object('migration', '671_phase3_parts_schema_foundation', 'source', 'parts_inventory')
...
SET ship_state = 'shipped',
    ...
    evidence_link = CASE
      WHEN COALESCE(evidence_link, '') LIKE '%supabase/migrations/671_phase3_parts_schema_foundation.sql%'
        THEN evidence_link
      ELSE COALESCE(NULLIF(evidence_link, ''), 'QEP_PHASE3_PARTS_BLUEPRINT_2026-05-26.md §1') ||
        ' | supabase/migrations/671_phase3_parts_schema_foundation.sql'
    END,
    notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '671_phase3_parts_schema_foundation.sql',
