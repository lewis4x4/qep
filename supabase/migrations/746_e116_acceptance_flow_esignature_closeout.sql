...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%746_e116_acceptance_flow_esignature_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'docs/adr/ADR-016-*') ||
      ...
      ' | supabase/migrations/711_a35_branded_acceptance_flow_closeout.sql' ||
      ' | docs/quote-flow-audit.md' ||
      ' | docs/quote-flow-backend-plan.md' ||
      ' | supabase/migrations/746_e116_acceptance_flow_esignature_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '746_e116_acceptance_flow_esignature_closeout.sql',
    ...
    'implementation_evidence', jsonb_build_array(
      ...
      'supabase/migrations/711_a35_branded_acceptance_flow_closeout.sql shipped the branded acceptance implementation that ADR-016 governs'
    ),
