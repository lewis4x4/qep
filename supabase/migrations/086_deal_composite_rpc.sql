-- ============================================================================
-- Migration 086: Deal Composite RPC
--
-- Single-query endpoint returning deal + stage + contact + company +
-- needs_assessment + cadences + touchpoints + demos + deposits in one
-- JSON response. Replaces 9 separate frontend queries.
-- ============================================================================
...
begin
  ...
  if v_deal ->> 'primary_contact_id' is not null then
    ...
    where c.id = (v_deal ->> 'primary_contact_id')::uuid and c.deleted_at is null;
  end if;
  ...
  if v_deal ->> 'company_id' is not null then
    ...
    where co.id = (v_deal ->> 'company_id')::uuid and co.deleted_at is null;
  end if;
