...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%762_f44_supersession_detection_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'supabase/migrations/597_qep_stream_f_decision_velocity.sql F4.4') ||
      ...
      ' | supabase/migrations/762_f44_supersession_detection_closeout.test.ts' ||
      ' | supabase/migrations/762_f44_supersession_detection_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '762_f44_supersession_detection_closeout.sql',
