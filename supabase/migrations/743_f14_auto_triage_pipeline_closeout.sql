...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%743_f14_auto_triage_pipeline_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'supabase/migrations/597_qep_stream_f_decision_velocity.sql F1.4') ||
      ...
      ' | supabase/migrations/743_f14_auto_triage_pipeline_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '743_f14_auto_triage_pipeline_closeout.sql',
