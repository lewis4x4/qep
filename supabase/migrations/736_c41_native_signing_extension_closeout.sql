...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%736_c41_native_signing_extension_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'JAR-106 packet') ||
      ...
      ' | supabase/migrations/736_c41_native_signing_extension_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '736_c41_native_signing_extension_closeout.sql',
