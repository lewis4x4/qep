...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%760_f42_decision_dependency_recompute_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'supabase/migrations/597_qep_stream_f_decision_velocity.sql F4.2') ||
      ...
      ' | supabase/migrations/760_f42_decision_dependency_recompute_closeout.test.ts' ||
      ' | supabase/migrations/760_f42_decision_dependency_recompute_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '760_f42_decision_dependency_recompute_closeout.sql',
