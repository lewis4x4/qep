...
VALUES
  ('rental_physical_utilization_pct', 'Rental physical utilization',
   ...
   'on-rent units ÷ rentable fleet × 100', 'rental', 'shared',
  ...
  ('rental_time_utilization_pct_30d', 'Rental time utilization (30d)',
   ...
   'Σ on-rent days ÷ (fleet × 30 − service-down days) × 100', 'rental', 'shared',
  ...
  ('rental_dollar_utilization_pct', 'Rental dollar utilization',
   ...
   'trailing-365d rental invoice revenue ÷ Σ fleet OEC × 100', 'rental', 'shared',
  ...
  ('rental_on_rent_revenue_run_rate', 'Rental on-rent run rate',
   ...
   'Σ monthly-equivalent agreed rates on on-rent contracts', 'rental', 'shared',
