...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%747_e21_ui_brand_guide_compliance_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'qep_brand_guide.pdf') ||
      ...
      ' | supabase/migrations/747_e21_ui_brand_guide_compliance_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '747_e21_ui_brand_guide_compliance_closeout.sql',
