...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%757_f32_ratify_silence_cron.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'supabase/migrations/597_qep_stream_f_decision_velocity.sql F3.2') ||
      ...
      ' | supabase/migrations/757_f32_ratify_silence_cron.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '757_f32_ratify_silence_cron.sql',
    ...
    'implementation_evidence', jsonb_build_array(
      ...
      'supabase/migrations/757_f32_ratify_silence_cron.sql schedules qep-ratify-silence-runner-hourly at 17 * * * * using the modern x-internal-service-secret cron pattern'
    ),
