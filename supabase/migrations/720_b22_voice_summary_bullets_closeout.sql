...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%720_b22_voice_summary_bullets_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'QEP-OMI-CONSOLIDATED-BUILD-PLAN.md VC-2') ||
      ...
      ' | supabase/migrations/720_b22_voice_summary_bullets_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '720_b22_voice_summary_bullets_closeout.sql',
