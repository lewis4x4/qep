...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%supabase/migrations/679_g131_inventory_counter_reporting.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'QEP_PHASE3_PARTS_BLUEPRINT_2026-05-26.md section 2 /v1/reports/inventory + /v1/reports/counter') ||
      ' | supabase/migrations/679_g131_inventory_counter_reporting.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '679_g131_inventory_counter_reporting.sql',
