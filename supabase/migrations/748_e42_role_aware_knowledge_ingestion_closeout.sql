...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%748_e42_role_aware_knowledge_ingestion_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'QEP-OMI-CONSOLIDATED-BUILD-PLAN.md KL-2') ||
      ...
      ' | supabase/migrations/748_e42_role_aware_knowledge_ingestion_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '748_e42_role_aware_knowledge_ingestion_closeout.sql',
