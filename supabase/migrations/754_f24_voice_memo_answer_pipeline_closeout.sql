...
SET
  ...
  evidence_link = CASE
    WHEN COALESCE(evidence_link, '') LIKE '%754_f24_voice_memo_answer_pipeline_closeout.sql%'
      THEN evidence_link
    ELSE COALESCE(NULLIF(evidence_link, ''), 'supabase/migrations/597_qep_stream_f_decision_velocity.sql F2.4') ||
      ...
      ' | supabase/migrations/754_f24_voice_memo_answer_pipeline_closeout.sql'
  END,
  notes = CASE
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'migration', '754_f24_voice_memo_answer_pipeline_closeout.sql',
