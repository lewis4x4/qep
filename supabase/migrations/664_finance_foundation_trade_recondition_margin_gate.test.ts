...
const migrationPath = join(
  ...
  "supabase",
  "migrations",
  "664_finance_foundation_trade_recondition_margin_gate.sql",
);
...
function withScratchPostgres(callback: (psql: (sql: string) => string) => void): void {
  ...
  const root = mkdtempSync(join(tmpdir(), "qep-finance-664-"));
...
describe("664_finance_foundation_trade_recondition_margin_gate.sql contract", () => {
...
postgresBehavior("664 finance trade recondition behavior on scratch Postgres", () => {
