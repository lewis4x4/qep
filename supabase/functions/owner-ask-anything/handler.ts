 ...
 *   retarget queries. Missing active workspace fails closed (403).
 * - No cron/service-role path — JWT operators only.
 */
...
export interface OwnerAskAnythingBody {
  question?: unknown;
    ...
    input_schema: {
      ...
      properties: {
        status: {
          type: "string",
          description:
            "e.g. 'open' (anything not closed), 'closed_won', 'closed_lost', or omit for all",
        },
...
export function resolveOwnerAskAnythingWorkspace(params: {
  ...
  // cannot retarget queries; active_workspace_id is authoritative.
  return { ok: true, workspaceId: callerWorkspaceId };
}

async function readBody(req: Request): Promise<OwnerAskAnythingBody> {
  ...
  return parsed as OwnerAskAnythingBody;
}
...
export async function executeTool(
  ...
  embedQuery: typeof embedText = embedText,
): Promise<unknown> {
...
export interface OwnerAskAnythingDependencies {
...
const defaultDependencies: OwnerAskAnythingDependencies = {
  createAdminClient,
  resolveCallerContext,
  callClaude,
  executeTool,
...
export async function handleOwnerAskAnything(
  req: Request,
  overrides: Partial<OwnerAskAnythingDependencies> = {},
): Promise<Response> {
  ...
  try {
    ...
    if (caller.isServiceRole) {
      return safeJsonError("JWT authentication required", 403, origin);
    }
    ...
    const workspaceSelection = resolveOwnerAskAnythingWorkspace({
    ...
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      for (const tu of toolUses) {
        ...
        try {
          resultData = await dependencies.executeTool(
            admin,
            workspaceId,
            tu.name,
            tu.input,
