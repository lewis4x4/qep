...
import {
  profileIsVisibleInCallerWorkspace,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // This user-scoped read applies customer_profiles_extended workspace RLS,
    // which is anchored to the same active workspace returned as auth.workspaceId.
    // It must happen before constructing or invoking any service-role client.
    const profileAllowed = await profileIsVisibleInCallerWorkspace(
      auth.supabase as unknown as ProfileReadClient,
      profileId,
    );
    if (!profileAllowed) {
    ...
    // get_health_score_with_deltas RPCs after workspace authorization.
    ...
    const { error: computeErr } = await adminClient.rpc(
    ...
    if (computeErr) {
      console.error(
        "[health-score-single] compute failed:",
        computeErr.message,
      );
    ...
    const { data: deltas, error: deltaErr } = await adminClient.rpc(
    ...
    if (deltaErr) {
      console.error(
        "[health-score-single] delta fetch failed:",
        deltaErr.message,
      );
