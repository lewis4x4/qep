...
export interface LaneClassifierRequest extends LaneClassificationInput {
  apply_update?: boolean;
  decision_id?: string | null;
  decision_code?: string | null;
}

export interface LaneClassifierDependencies {
...
const defaultDependencies: LaneClassifierDependencies = {
...
async function fetchOpenDecision(
...
async function applyLaneUpdate(
  admin: SupabaseClient,
  params: {
    lane: DecisionLane;
    decisionId?: string | null;
...
export async function handleLaneClassifier(
  req: Request,
  dependencies: Partial<LaneClassifierDependencies> = {},
): Promise<Response> {
  const { createAdminClient, isServiceRoleCaller: checkServiceRoleCaller } = {
    ...defaultDependencies,
    ...dependencies,
  };
  ...
  try {
    ...
    const fetchedDecision = await fetchOpenDecision(admin, {
    ...
    if (body.apply_update === true) {
      updatedDecision = await applyLaneUpdate(admin, {
