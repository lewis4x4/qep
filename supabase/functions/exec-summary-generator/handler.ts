...
export type AuthResult =
  | { ok: false }
  | { ok: true; isInternal: true }
  | { ok: true; isInternal: false; userId: string; workspace?: string | null };

export function resolveExecSummaryWorkspace(params: {
  isInternalCaller: boolean;
  authWorkspace?: string | null;
  requestedWorkspaceId?: string | null;
}): string {
  if (params.isInternalCaller) {
    return params.requestedWorkspaceId ?? "default";
  }
  return params.authWorkspace ?? "default";
}

export async function isAuthorizedCaller(
  req: Request,
  admin: SupabaseClient,
  internalSecret = INTERNAL_SECRET,
): Promise<AuthResult> {
  const secretHeader = req.headers.get("x-internal-service-secret");
  if (secretHeader && internalSecret && secretHeader === internalSecret) {
...
export function formatValue(metricKey: string, value: number | null): string {
...
export function evaluateBand(value: number | null, config: Record<string, unknown>): "good" | "warn" | "critical" | "neutral" {
...
export function buildBriefing(role: string, definitions: Definition[], snapshots: Snapshot[], alerts: Alert[]): string {
...
export async function handleExecSummaryGenerator(
  req: Request,
  overrides: Partial<ExecSummaryGeneratorDependencies> = {},
): Promise<Response> {
  const { createAdminClient, isAuthorizedCaller: authorizeCaller } = {
    ...defaultDependencies,
    ...overrides,
  };
  ...
  const auth = await authorizeCaller(req, admin);
  if (!auth.ok) {
  ...
  const workspace = resolveExecSummaryWorkspace({
    isInternalCaller: auth.isInternal,
    authWorkspace: auth.isInternal ? null : auth.workspace,
