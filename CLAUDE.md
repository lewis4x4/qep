...
2. `Transformation`: The segment includes or enables a capability that is materially beyond commodity CRM behavior.
...
- Product track: QEP OS, with active parallel streams:
  - CRM Phase 1 (HubSpot replacement)
...
- Add indexes with explicit purpose and avoid unbounded table scans on CRM list views.
...
- Keep idempotency and auditability for CRM import/sync paths.
...
- Prefer feature-local API adapters for CRM behavior.
- Mutation-heavy workflows should go through CRM router contracts.
